...
void Ecriture_Disque(int m, int n, int nbcoul, int graine, Solution *S, char *type){
  char filename[104];
  char inttochar[6];
  
  strcpy(filename,"../solutions/");
  strcat(filename,type);
  strcat(filename,"_Robot_");
...
void Solution_desallocation(Solution *S){
  Cell_char *cour = S->prem, *tmp;
  while (cour != NULL) {
    tmp = cour;
    cour = cour->suiv;
    free(tmp);
