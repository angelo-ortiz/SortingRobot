...
void AffGrille_init(Grille *G, int taille_pixel, AffGrille *AG){
  int i;
  ...
  for (i=0;i<32;i++){ // pour sauter les premieres couleurs un peu palotes
    genecl=genecl*(i+i+1);
  ...
  // Dans le cas contraire, une case sera representee par un seul rectangle

  if ((taille_pixel*1.0)/G->n>1){
    AG->cas_aff=0;
    AG->taille_case=taille_pixel/G->n;
  }
  else{
    AG->cas_aff=1;
    AG->modulo=G->n/taille_pixel;
  }
...
void AffGrille_ouvre_fenetre(AffGrille *AG){
 ...
 // Initialize SDL 2D display component.
  ...
  SDL_SetVideoMode(AG->taille_case*AG->G->n, AG->taille_case*AG->G->m, 0, flags);
...
void AffGrille_redessine_case(AffGrille *AG, Grille *G, int i, int j){
  ...
  if (AG->cas_aff==0){ // Si affiche d une case possible
    ...
    if (G->T[i][j].fond!=G->T[i][j].piece) {
      ...
      SDL_Rect piece = {(j+0.25)*AG->taille_case, (i+0.25)*AG->taille_case,
			0.5*AG->taille_case, 0.5*AG->taille_case};
      
      if (G->T[i][j].piece!=-1) // Si absence de piece
	SDL_FillRect(screen, &piece, AG->T_coul[G->T[i][j].piece]);
      else
	SDL_FillRect(screen, &piece, 0);
    }    
    else{ // Si case noire
      SDL_Rect rectangle = {j*AG->taille_case, i*AG->taille_case,
			    AG->taille_case, AG->taille_case};
    ...
    SDL_Rect robotext = {(j+0.35)*AG->taille_case, (i+0.35)*AG->taille_case,
      		         0.30*AG->taille_case, 0.30*AG->taille_case};
    SDL_Rect robot = {(j+0.375)*AG->taille_case, (i+0.375)*AG->taille_case,
  ...
  else{ // Si affichage d une case impossible
