...
\begin{tikzpicture}[x=0.05cm,y=0.4cm]
  ...
  % grid
%  \draw[style=help lines, ystep=2, xstep=50] (\xmin,\ymin) grid
%  (\xmax,\ymax);
  ...
  %labels
  \node[below=0.8cm] at (150,0) {Nombre de couleurs};
  \node[rotate=90, above=0.8cm] at (-13,5) {Temps d'ex\'ecution[s]};  
    
  % plot the data from the file data.dat
  % smooth the curve and mark the data point with a dot
  \draw[color=blue] plot[smooth,mark=*,mark size=1pt] file {../data/colour/300_avl.txt}
   node [right] {AVL};
  ...
  \draw[color=red] plot[smooth,mark=*,mark size=1pt] file {../data/colour/300_couleur.txt}
   node [right] {Couleur};  
