...
extern sr_algorithme app_algorithmes[NB_APP];
...
int main (int argc, char* *argv)
{
  ...
  Grille G;
  //Solution S = NULL;
  char *res_algo[NB_APP] = {"../data/size/naif.txt", "../data/size/circulaire.txt", \
  ...
  do {
    ...
    for (i = 0; i < NB_APP; ++i) {
      if (!fin_algo[i]) {
	temps_moy = test_algorithme(f_algo[i], app_algorithmes[i], &G, NULL, n);
